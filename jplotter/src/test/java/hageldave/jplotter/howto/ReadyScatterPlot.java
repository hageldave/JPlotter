...
//    	minimal();
	...
	public static void minimal() {
		...
		for (int label : new int[] { 0, 1, 2 }) {
			double[][] chunk = IntStream.range(0, dataset.getNumPoints()).filter(i -> dataset.getLabels()[i] == label)
					.mapToObj(i -> dataset.getData()[i]).toArray(double[][]::new);
	...
	public static void sophisticated() throws IOException {
		...
		String[] classLabels = new String[] { "Rad Flow", "Fpv Close", "Fpv Open", "High", "Bypass", "Bpv Close",
				"Bpv Open", };
		for (int i = 0; i < classLabels.length; i++)
			data.add(new LinkedList<>());

		URL statlogsrc = new URL(
				"https://archive.ics.uci.edu/ml/machine-learning-databases/statlog/shuttle/shuttle.tst");
		try (InputStream stream = statlogsrc.openStream(); Scanner sc = new Scanner(stream)) {
		...
		plot.setVisualMapping(new ScatterPlotVisualMapping() {
			// standard scatter plot glyphs excluding CROSS because we can't easily draw an
			// outline for CROSS
			Glyph[] glyphs = new Glyph[] { DefaultGlyph.CIRCLE_F, DefaultGlyph.SQUARE_F, DefaultGlyph.TRIANGLE_F,
		...
		SimpleSelectionModel<String> activeTools = new SimpleSelectionModel<>();
		final String TOOL_PAN = "pan";
		final String TOOL_HOVER = "hover";
		final String TOOL_RECT_ZOOM = "rect_zoom";
		final String TOOL_SCROLL_ZOOM = "scroll_zoom";
		final String TOOL_RECT_SELECT = "rect_point_selection";
		final String TOOL_ROPE_SELECT = "rope_point_selection";
		final String TOOL_LASSO_SELECT = "lasso_point_selection";
		activeTools.setSelection(TOOL_PAN, TOOL_SCROLL_ZOOM, TOOL_HOVER);

		BiFunction<Integer, String, KeyMaskListener> keyBindingFunction = (key, tool) -> new KeyMaskListener(key)
				.addStateChangedCallback((k, pressed) -> {
					if (pressed) {
						ArrayList<String> selection = new ArrayList<>(activeTools.getSelection());
						selection.add(tool);
						activeTools.setSelection(selection);
					} else {
						ArrayList<String> selection = new ArrayList<>(activeTools.getSelection());
		...
		plot.getCanvas().asComponent().addKeyListener(keyBindingFunction.apply(0, TOOL_PAN));
		plot.getCanvas().asComponent().addKeyListener(keyBindingFunction.apply(0, TOOL_HOVER));
		plot.getCanvas().asComponent().addKeyListener(keyBindingFunction.apply(KeyEvent.VK_SHIFT, TOOL_RECT_ZOOM));
		plot.getCanvas().asComponent().addKeyListener(keyBindingFunction.apply(KeyEvent.VK_S, TOOL_RECT_SELECT));
		plot.getCanvas().asComponent().addKeyListener(keyBindingFunction.apply(KeyEvent.VK_R, TOOL_ROPE_SELECT));
		plot.getCanvas().asComponent().addKeyListener(keyBindingFunction.apply(KeyEvent.VK_L, TOOL_LASSO_SELECT));
		
		activeTools.addSelectionListener(s -> {
			System.out.println("active tools: " + s.stream().sorted().collect(Collectors.joining(", ")));
		});
		...
		plot.getCanvas().asComponent().addMouseListener(new MouseAdapter() {
			...
			public void mouseEntered(MouseEvent e) {
				plot.getCanvas().asComponent().requestFocus();
			}
		});

		plot.addPanning(() -> activeTools.getSelection().contains(TOOL_PAN));
		plot.addRectangleSelectionZoom(() -> activeTools.getSelection().contains(TOOL_RECT_ZOOM));
		plot.addScrollZoom(() -> activeTools.getSelection().contains(TOOL_SCROLL_ZOOM));
		plot.addRectangularPointSetSelector(() -> activeTools.getSelection().contains(TOOL_RECT_SELECT));
		plot.addRopePointSetSelector(() -> activeTools.getSelection().contains(TOOL_ROPE_SELECT));
		plot.addLassoPointSetSelector(() -> activeTools.getSelection().contains(TOOL_LASSO_SELECT));

		// add key bindings for toggling interaction tools
		...
		JTable datasetTable = new JTable(new TableModel() {
			...
			public void addTableModelListener(TableModelListener l) {
				// creating a listener for the plot's data model instead since its the source of
				// the table model
		...
		SimpleSelectionModel<Pair<Integer, Integer>> selectedDataPoints = new SimpleSelectionModel<Pair<Integer, Integer>>();
		{
			selectedDataPoints.addSelectionListener(s -> {
				int[] selectedRows = s.stream()
						.mapToInt(pair -> plot.getDataModel().getGlobalIndex(pair.first, pair.second)).toArray();
		...
		datasetTable.getSelectionModel().addListSelectionListener(e -> {
			...
			List<Pair<Integer, Integer>> selectedInstances = Arrays.stream(selectedRows)
					.mapToObj(i -> plot.getDataModel().locateGlobalIndex(i)).collect(Collectors.toList());
		...
		plot.addPointSetSelectionListener(new PointSetSelectionListener() {
			...
			public void onPointSetSelectionChanged(ArrayList<Pair<Integer, TreeSet<Integer>>> selectedPoints,
				...
				List<Pair<Integer, Integer>> toAccentuate = selectedPoints.stream()
						.flatMap(pair -> pair.second.stream().map(i -> Pair.of(pair.first, i)))
		...
		plot.addScatterPlotMouseEventListener(new ScatterPlotMouseEventListener() {
			...
			@Override
			public void onInsideMouseEventPoint(String mouseEventType, MouseEvent e, Point2D coordsysPoint,
					int chunkIdx, int pointIdx) {
				/* mouse interacting with point in the coordinate system */
				
				if(!activeTools.getSelection().contains(TOOL_HOVER)) {
					return;
				}
...
//        			// remove highlight since no point is under cursor
			...
			public void onOutsideMouseEventElement(String mouseEventType, MouseEvent e, int chunkIdx) {
				/*
				 * mouse interacting with element outside the coordinate system, e.g. legend
				 * element
				 */
				if(!activeTools.getSelection().contains(TOOL_HOVER)) {
					return;
				...
				if (mouseEventType != MOUSE_EVENT_TYPE_MOVED)
					return;
				// on mouse over legend element of chunk: desaturate every point chunk except
				// corresponding chunk
				List<Pair<Integer, Integer>> instancesOfChunk = IntStream
						.range(0, plot.getDataModel().chunkSize(chunkIdx)).mapToObj(i -> Pair.of(chunkIdx, i))
						.collect(Collectors.toList());
