...
public abstract class CoordSysMouseOver extends MouseAdapter {
    ...
    protected PickingRegistry<Points.PointDetails> registry;
    ...
    public CoordSysMouseOver(JPlotterCanvas canvas, CoordSysRenderer coordsys,
                             LinkedList<double[][]> allDataPoints, PickingRegistry<Points.PointDetails> registry) {
        this.canvas = canvas;
    ...
    protected boolean findPoints(final MouseEvent e) {
        Points.PointDetails details = this.registry.lookup(this.canvas.getPixel(e.getX(), e.getY(), true, 5));
        if (details != null) {
