...
/**
 * The {@link CoordSysLassoSelector} extends {@link CoordSysViewSelector} to
 * provide free-form lasso selection for the coordinate view of a
 * {@link CoordSysRenderer}.
 * <p>
 ...
 * {@link Path2D#WIND_EVEN_ODD} is used so inside/outside follows the lasso
 * outline.
 * <p>
 * Interaction model:
 * <ul>
 * <li>Press key mask + left mouse button inside the coordsys area to start a
 * new lasso.</li>
...
public abstract class CoordSysLassoSelector extends CoordSysViewSelector implements KeyListener {
	...
	/**
	 * Creates a new {@link CoordSysLassoSelector} using the default {@code L} key
	 * mask.
	 *
	 * @param canvas   the canvas displaying the coordsys
	 * @param coordsys the coordinate system to apply the lasso selection in
	...
	/**
	 * Will be called when lasso selection is completed (mouse button released). The
	 * path uses {@link Path2D#WIND_EVEN_ODD}, so
