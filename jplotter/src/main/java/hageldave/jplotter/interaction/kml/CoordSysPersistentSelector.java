...
/**
 * The CoordSysPersistentSelector extends {@link CoordSysViewSelector} to
 * provide a persistent rectangular selection that survives mouse release and
 * can be moved and resized by subsequent drag interactions.
 * <p>
 * The selection rectangle is stored in coordinate space (not pixel space), so
 * it remains correct when the user pans or zooms.
 * <p>
 * Interaction model:
 * <ul>
 ...
 * <li>SHIFT+click or SHIFT+drag outside the selection clears it and starts a
 * new one.</li>
 ...
 * <ul>
 * <li>{@link #areaSelectedOnGoing(double, double, double, double)} fires
 * continuously during any drag (drawing, moving, or resizing).</li>
 * <li>{@link #areaSelected(double, double, double, double)} fires once on mouse
 * release.</li>
 * </ul>
 * <p>
 * Intended use:
 ...
 * </pre>
 * <p>
 * Hit test zones use a band of {@code 6px} outward and {@code 1px} inward
 * around each border edge. Corners take priority over edges in the hit test,
 * and the interior of the {@code 1px} inset activates the MOVE cursor.
 *
 * @author hageldave
 */
public abstract class CoordSysPersistentSelector extends CoordSysViewSelector {
	...
	/**
	 * Drag interaction modes for the persistent selector. {@code NONE} means no
	 * active drag. {@code DRAWING} means a new rectangle is being drawn.
	 * {@code MOVING} and the {@code RESIZE_*} variants are entered when an existing
	 * selection is dragged from the interior or from an edge/corner, respectively.
	 */
	protected enum DragMode {
		NONE, DRAWING, MOVING, RESIZE_N, RESIZE_S, RESIZE_E, RESIZE_W, RESIZE_NE, RESIZE_NW, RESIZE_SE, RESIZE_SW
	}
	...
	/**
	 * X component of the coord-space drag anchor (set on mousePressed for
	 * MOVING/RESIZING).
	 */
	protected double anchorCoordX;
	/**
	 * Y component of the coord-space drag anchor (set on mousePressed for
	 * MOVING/RESIZING).
	 */
	...
	/**
	 * Creates a new {@link CoordSysPersistentSelector} for the specified canvas and
	...
	/**
	 * Creates a new {@link CoordSysPersistentSelector} using the default SHIFT key
	 * mask.
	 *
	 * @param canvas   the canvas displaying the coordsys
	 * @param coordsys the coordinate system to apply the selection in
	...
	public void mouseDragged(MouseEvent e) {
		...
		if (dragMode == DragMode.DRAWING) {
			...
			areaSelectedOnGoing(Math.min(p1.getX(), p2.getX()), Math.min(p1.getY(), p2.getY()),
	...
	/**
	 * Updates the canvas cursor based on the current mouse position relative to the
	 * persistent selection. Only activates when the key mask is pressed and a
	...
	/**
	 * Performs a hit test of a given AWT pixel coordinate against the current
	 * selection rectangle, returning the {@link DragMode} corresponding to the hit
	 * zone.
	 * <p>
	 * Hit zones use a band of {@code 6px} outward and {@code 1px} inward from each
	 * border edge. Corners (intersections of two edge bands) take priority over
	 * plain edges, and the interior (strictly inside the {@code 1px} inset on all
	 * sides) maps to {@link DragMode#MOVING}. Points outside the {@code 6px}
	 * expanded bounding box map to {@link DragMode#NONE}.
	 *
	 * @param mouseAWT the mouse position in AWT pixel coordinates
	 * @return the hit zone as a {@link DragMode}, or {@link DragMode#NONE} if
	 *         outside
	 */
	protected DragMode hitTest(Point mouseAWT) {
		...
		Point2D tlAWT = coordsys.transformCoordSys2AWT(new Point2D.Double(selMinX, selMaxY), canvas.getHeight());
		Point2D brAWT = coordsys.transformCoordSys2AWT(new Point2D.Double(selMaxX, selMinY), canvas.getHeight());
	 ...
	 * stored {@link #selMinX}/{@link #selMinY}/{@link #selMaxX}/{@link #selMaxY}
	 * are used directly, since the overlay is an {@code AdaptableView} whose
	 * segments are interpreted in coordinate space.
	 */
	@Override
	...
	/**
	 * {@inheritDoc} Also registers the ESC key listener so pressing ESC calls
	 * {@link #clearSelection()}.
	 */
	@Override
	...
	/**
	 * {@inheritDoc} Also removes the ESC key listener.
	 */
	@Override
	...
	/**
	 * Will be called when selection is cleared (i.e. by ESC or clicking outside).
	 */
