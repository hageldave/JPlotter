...
public class CurveRenderer extends GenericRenderer<Curves> {
	...
	protected static final String geometryShaderSrc = ""
			...
			+ NL + "   vec4 dt_startEnd = vec4(normalize(start.xw),normalize(end.xw));"
			+ NL + "   float extend = 0.5*linewidthMultiplier;"
			+ NL + "   "
			+ NL + "   vec2 dir1 = dt_startEnd.xy;"
			+ NL + "   vec2 dir2 = normalize(p1-p2);"
			+ NL + "   vec2 q = p1;"
			+ NL + "   "
			+ NL + "   vec2 dir12 = dir1+dir2;"
			+ NL + "   vec2 miter = vec2(dir1.y, -dir1.x);"
			+ NL + "   float dotm = dot(miter,dir12);"
			+ NL + "   miter = abs(dotm) < 1e-5 ? miter : (1/dotm)*dir12;"
			+ NL + "   "
			...
			+ NL + "   "
			+ NL + "   dir1 = -dir2;"
			+ NL + "   dir2 = -dt_startEnd.zw;"
			+ NL + "   q = p2;"
			+ NL + "   "
			+ NL + "   dir12 = dir1+dir2;"
			+ NL + "   miter = vec2(dir1.y, -dir1.x);"
			+ NL + "   dotm = dot(miter,dir12);"
